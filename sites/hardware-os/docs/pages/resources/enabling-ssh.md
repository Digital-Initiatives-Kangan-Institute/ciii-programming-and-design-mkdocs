...
8. After the Pi restarts, SSH will be enabled. Find the Pi's IP address — see [Using ifconfig](ifconfig.md) — then connect remotely:
    ```bash
...
- After enabling SSH, find the Pi's IP address (see [Using ifconfig](ifconfig.md)) and connect remotely with `ssh <username>@<ip-address>`
